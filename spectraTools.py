...
def mean_cpl(vspec, vwaves, refwvl, contint):
    """
    ...
    :param vspec: numpy.ndarray
        1-D numpy array of Stokes-V profile
    :param vwaves: numpy.ndarray
        Corresponding wavelengths
    :param refwvl: float
        Reference wavelength
    :param contint: float
        Mean Stokes-I continuum intensity
    :return mean_circular_polarization: float
    """
    sign_vector = np.zeros(len(vspec))
    for i in range(len(vspec)):
        if vwaves[i] < refwvl:
            sign_vector[i] = -1
        elif vwaves[i] > refwvl:
            sign_vector[i] = 1
    mean_circular_polarization = (1/(len(vwaves) * contint)) * np.nansum(
        sign_vector * np.abs(vspec)
    )
    return mean_circular_polarization


def mean_lpl(qspec, uspec, contint):
    """
    Computes mean linear polarization as described by Martinez Pillet (2011).
    :param qspec: numpy.ndarray
        Stokes-Q vector
    :param uspec:  numpy.ndarray
        Stokes-U vector
    :param contint: float
        Mean Stokes-I continuum intensity
    :return: float
